--!nocheck
-- Compkiller UI Library (extracted) --
...
local Compkiller = {
	...
	CacheDirectory = "Compkiller",
	__KEYBINDS_CACHE = nil,
	NOTIFY_CACHE = nil,
	IS_DRAG_MOVE = false,
	IsDrage = false,
	Version = '2.6',
	Logo = "rbxassetid://17691279345",
	Windows = {},
	Scale = {
	...
	ProtectGui = getgenv().protect_gui or getgenv().protectgui or (getgenv().syn and getgenv().syn.protect_gui) or function(s) return s; end,
};

-- ============================================================
-- Robust Logo Handling
-- ============================================================
function Compkiller:_ResolveLogo(id)
	local value = tostring(id or self.Logo or "");
	local assetId = string.match(value, "%d+");
	if assetId then
...
function Compkiller:_PreloadLogo(id)
	local asset = self:_ResolveLogo(id);
...
function Compkiller:_SetLogo(image, id, visible)
	...
	image.ImageTransparency = visible == false and 1 or 0;
	image.Image = asset;

	-- Second path for environments where the direct rbxassetid URI is slow to resolve.
	if numericId then
		task.spawn(function()
			local loaded = false;
			...
			local connection;
			pcall(function()
				connection = image:GetPropertyChangedSignal("IsLoaded"):Connect(function()
					changed = image.IsLoaded;
				end);
			end);

			task.wait(1.5);
			local stillUnloaded = true;
			pcall(function() stillUnloaded = not image.IsLoaded; end);

			if stillUnloaded and not changed then
				image.Image = string.format("rbxthumb://type=Asset&id=%s&w=420&h=420", numericId);
			end;
...
Compkiller.Logo = Compkiller:_ResolveLogo(Compkiller.Logo);
task.spawn(function()
	Compkiller:_PreloadLogo(Compkiller.Logo);
end);

Compkiller.Colors = {
	Highlight = Color3.fromRGB(150, 90, 255),      -- Elegantes, modernes Premium-Violett/Lila (Akzent)
	Toggle = Color3.fromRGB(120, 60, 225),         -- Passendes dunkleres Violett für aktivierte Schalter
	Risky = Color3.fromRGB(255, 100, 100),         -- Edles Soft-Rot für Warnungen (kein aggressives Signalgelb)
	BGDBColor = Color3.fromRGB(10, 10, 13),        -- Ultra-Deep Black/Onyx für den Haupt-Hintergrund
	BlockColor = Color3.fromRGB(18, 18, 23),       -- Dunkles Anthrazit für die Sections (hebt sich dezent ab)
	StrokeColor = Color3.fromRGB(28, 28, 36),      -- Sehr feine, minimalistische Rahmenlinien (Outlines)
	SwitchColor = Color3.fromRGB(245, 245, 250),   -- Klares Off-White/Eisweiß für perfekte Text-Lesbarkeit
	DropColor = Color3.fromRGB(24, 24, 31),        -- Hintergrund für Dropdowns und Slider-Bahnen
	MouseEnter = Color3.fromRGB(36, 36, 47),       -- Subtiler, heller Aufleucht-Effekt beim Drüberfahren
	BlockBackground = Color3.fromRGB(15, 15, 20),  -- Hintergrund für Pop-ups und den Farbwähler
	LineColor = Color3.fromRGB(32, 32, 42),        -- Sehr dezente Trennlinien zwischen den Optionen
	HighStrokeColor = Color3.fromRGB(48, 42, 68),  -- Rahmenfarbe mit leichtem lila Schimmer für aktive Elemente
};
...
function Compkiller:_IsMouseOverFrame(Frame)
	...
	if not Mouse or not AbsPos or not AbsSize then
		return false;
	end;
...
function Compkiller:Security(directory,Config) -- Security Mode
...
function Compkiller:_DrawKeybinds(Window)
...
function Compkiller:_KeybindHandler(Parent, ObjectType, ElementAPI, Signal , Zindex, ElementCFG)
...
function Compkiller:_LoadElement(Parent, EnabledLine, Signal , DisableStackKeybind)
	...
	function Args:AddParagraph(Config) -- request by Neptune
		...
		local Args = {};
		local TextUpdateSignal = Instance.new("BindableEvent");

		local function _SafeParagraphText(value)
			if value == nil then
				return "";
			end;
		...
		local function _ApplyParagraphText(Label, value)
			local text = _SafeParagraphText(value);
		...
		local TextUpdateConnection = TextUpdateSignal.Event:Connect(function(kind, value)
			if kind == "Title" then
				_ApplyParagraphText(BlockText, value);
			elseif kind == "Content" then
				_ApplyParagraphText(DescriptionText, value);
			end;
		end);

		local function _SetParagraphText(kind, Label, value)
			local text = _SafeParagraphText(value);
			local ok = pcall(function()
				TextUpdateSignal:Fire(kind, text);
			end);

			if not ok then
				_ApplyParagraphText(Label, text);
			end;
		end;

		Args.TextUpdateSignal = TextUpdateSignal;
		Args.TextUpdateConnection = TextUpdateConnection;

		function Args:SetTitle(title)
			_SetParagraphText("Title", BlockText, title);
		end;

		function Args:SetContent(content)
			_SetParagraphText("Content", DescriptionText, content);
		end;
...
function Compkiller.new(Config)
	...
	Compkiller:_SetLogo(CompLogo, Config.Logo)
			...
			for i,v in next ,Childrens do task.wait();
				if v:IsA('Frame') then
					if v.LayoutOrder > last then
						scale = scale + v.AbsoluteSize.Y + Offset;
		...
		function TabArgs:DrawSection(config)
			...
			local refreshScale = function()
				...
				for i,v in next , Childrens do task.wait();
					if v:IsA('Frame') then
						if v ~= Section then
							frameFound = frameFound + 1;
							allscale = allscale + v:GetAttribute('HEIGHTSCALE') or v.AbsoluteSize.Y;
				...
				if Latest >= frameFound then
					...
					if allscale >= (Parent.AbsoluteSize.Y - lscale) or UIListLayout.AbsoluteContentSize.Y >= (Parent.AbsoluteSize.Y - lscale) then
						...
						for i,v in next , Parent:GetChildren() do
							if v:IsA('Frame') then
								parentScale = parentScale + v:GetAttribute('HEIGHTSCALE') + ParentLayout.Padding.Offset;
							end;
						end;
	...
	do
		...
		Compkiller:_SetLogo(ImageLabel, Config.Logo)
...
function Compkiller:Loader(IconId,Duration)
	...
	Icon.ZIndex = 100
	Compkiller:_SetLogo(Icon, IconId or Compkiller.Logo);
	Icon.ImageTransparency = 1
...
function Compkiller:KeySystem(Config)
	Config = Compkiller.__CONFIG(Config, {
		Name = "Key System",
		ServiceId = "",
		Logo = Compkiller.Logo,
	...
	Content.Name = Compkiller:_RandomString()
	...
	AvatarFrame.Name = Compkiller:_RandomString()
	AvatarFrame.Parent = Content
	AvatarFrame.BackgroundColor3 = Compkiller.Colors.BlockColor
	...
	AvatarStroke.Color = Compkiller.Colors.Highlight
	AvatarStroke.Parent = AvatarFrame

	AvatarImage.Name = Compkiller:_RandomString()
	...
	pcall(function()
		local img, ready = Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
		if ready then
	...
	Title.Name = Compkiller:_RandomString()
	...
	Title.TextSize = 26
	Title.TextColor3 = Compkiller.Colors.SwitchColor
	Title.Text = Config.Name

	local WelcomeText = Instance.new("TextLabel")
	WelcomeText.Name = Compkiller:_RandomString()
	WelcomeText.Parent = Content
	WelcomeText.BackgroundTransparency = 1
	WelcomeText.Size = UDim2.new(0, 260, 0, 22)
	WelcomeText.LayoutOrder = 3
	WelcomeText.Font = Enum.Font.Gotham
	WelcomeText.TextSize = 17
	WelcomeText.TextColor3 = Compkiller.Colors.SwitchColor
	WelcomeText.TextTransparency = 0.4
	WelcomeText.Text = "Welcome, " .. LocalPlayer.Name

	local KeyStatus = Instance.new("TextLabel")
	KeyStatus.Name = Compkiller:_RandomString()
	KeyStatus.Parent = Content
	KeyStatus.BackgroundTransparency = 1
	KeyStatus.Size = UDim2.new(0, 260, 0, 20)
	KeyStatus.LayoutOrder = 4
	KeyStatus.Font = Enum.Font.GothamBold
	KeyStatus.TextSize = 15
	KeyStatus.TextColor3 = Color3.fromRGB(255, 93, 93)
	KeyStatus.Text = ""

	local GetKeyButton = Instance.new("TextButton")
	local GetKeyCorner = Instance.new("UICorner")

	GetKeyButton.Name = Compkiller:_RandomString()
	GetKeyButton.Parent = Content
	GetKeyButton.BackgroundColor3 = Compkiller.Colors.BlockColor
	GetKeyButton.BackgroundTransparency = 0.2
	GetKeyButton.BorderSizePixel = 0
	GetKeyButton.Size = UDim2.new(0, 220, 0, 42)
	GetKeyButton.LayoutOrder = 5
	GetKeyButton.Font = Enum.Font.GothamBold
	GetKeyButton.TextSize = 16
	GetKeyButton.TextColor3 = Compkiller.Colors.SwitchColor
	GetKeyButton.Text = "Get Key"

	GetKeyCorner.CornerRadius = UDim.new(0, 10)
	GetKeyCorner.Parent = GetKeyButton

	Compkiller:_Hover(GetKeyButton, function()
		Compkiller:_Animation(GetKeyButton, TweenInfo.new(0.2), { BackgroundTransparency = 0 })
	end, function()
		Compkiller:_Animation(GetKeyButton, TweenInfo.new(0.2), { BackgroundTransparency = 0.2 })
	end)

	local KeyBox = Instance.new("TextBox")
	local KeyBoxCorner = Instance.new("UICorner")

	KeyBox.Name = Compkiller:_RandomString()
	KeyBox.Parent = Content
	KeyBox.BackgroundColor3 = Compkiller.Colors.DropColor
	KeyBox.BackgroundTransparency = 0.3
	KeyBox.BorderSizePixel = 0
	KeyBox.Size = UDim2.new(0, 220, 0, 42)
	KeyBox.LayoutOrder = 6
	KeyBox.Font = Enum.Font.Gotham
	KeyBox.TextSize = 16
	KeyBox.TextColor3 = Compkiller.Colors.SwitchColor
	KeyBox.PlaceholderColor3 = Compkiller.Colors.SwitchColor
	...
	RedeemButton.Name = Compkiller:_RandomString()
	RedeemButton.Parent = Content
	RedeemButton.BackgroundColor3 = Compkiller.Colors.Highlight
	RedeemButton.BackgroundTransparency = 0.1
	RedeemButton.BorderSizePixel = 0
	RedeemButton.Size = UDim2.new(0, 220, 0, 42)
	RedeemButton.LayoutOrder = 7
	RedeemButton.Font = Enum.Font.GothamBold
	RedeemButton.TextSize = 16
	RedeemButton.TextColor3 = Compkiller.Colors.SwitchColor
	RedeemButton.Text = "Redeem"

	RedeemCorner.CornerRadius = UDim.new(0, 10)
	RedeemCorner.Parent = RedeemButton

	Compkiller:_Hover(RedeemButton, function()
		Compkiller:_Animation(RedeemButton, TweenInfo.new(0.2), { BackgroundTransparency = 0 })
	end, function()
		Compkiller:_Animation(RedeemButton, TweenInfo.new(0.2), { BackgroundTransparency = 0.1 })
	end)

	local LoadingSection = Instance.new("Frame")
	LoadingSection.Name = Compkiller:_RandomString()
	LoadingSection.Parent = Content
	LoadingSection.Size = UDim2.new(0, 220, 0, 40)
	LoadingSection.LayoutOrder = 8
	...
	Bar.Name = Compkiller:_RandomString()
	Bar.Parent = LoadingSection
	...
	Fill.Name = Compkiller:_RandomString()
	Fill.Parent = Bar
	...
	PercentText.Name = Compkiller:_RandomString()
	PercentText.Parent = LoadingSection
	PercentText.BackgroundTransparency = 1
	PercentText.Size = UDim2.new(1, 0, 0, 24)
	PercentText.Position = UDim2.new(0, 0, 0, 12)
	PercentText.Font = Enum.Font.GothamBold
	PercentText.TextSize = 15
	PercentText.TextColor3 = Compkiller.Colors.SwitchColor
	...
	function ReturnRef:Show()
		ScreenGui.Enabled = true
		MainFrame.Size = UDim2.new(0, 0, 0, 0)
		Compkiller:_Animation(MainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 485, 0, 426)
		})
	end;

	function ReturnRef:Hide()
		Compkiller:_Animation(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {
	...
	function ReturnRef:Destroy()
		ScreenGui:Destroy()
	end;

	local INVALID_COLOR = Color3.fromRGB(255, 92, 92)
	local VALID_COLOR = Color3.fromRGB(110, 230, 130)
	local MIN_TIME = 3.5
	...
	local function collectAssets()
		...
		for _, desc in game:GetDescendants() do
			...
			elseif desc:IsA("Audio") then
				table.insert(assets, desc.SoundId)
	...
	local function startLoading()
		...
		KeyStatus.TextColor3 = VALID_COLOR
		...
		if #assets > 0 then
			task.spawn(function()
				pcall(function()
					ContentProvider:PreloadAsync(assets, function(_, loaded, failed)
						preloaded = loaded + failed
					end)
				end)
				finished = true
			end)
		...
		while not finished do
			local elapsed = (os.clock() - startTime) / MIN_TIME
			local realProgress = #assets > 0 and (preloaded / #assets) or 0
			local progress = math.max(math.min(realProgress, 1), math.min(elapsed, 1))
			local percent = math.floor(progress * 100)
			Compkiller:_Animation(Fill, TweenInfo.new(0.05), { Size = UDim2.fromScale(progress, 1) })
			PercentText.Text = percent .. "%"
			task.wait(0.05)
		end

		Compkiller:_Animation(Fill, TweenInfo.new(0.1), { Size = UDim2.fromScale(1, 1) })
	...
	RedeemButton.MouseButton1Click:Connect(function()
		...
		if key == "" then
			...
			KeyStatus.TextColor3 = INVALID_COLOR
			return
		end

		KeyStatus.Text = "Validating..."
		KeyStatus.TextColor3 = Compkiller.Colors.SwitchColor
